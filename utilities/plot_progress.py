...
parser.add_argument('-path_protein','--project',required=True,help='Location of project, i.e file_path+protein')
parser.add_argument('-sz_test','--size_test_set',required=True,help='Number of molecules in the test set')
parser.add_argument('-it_start','--start_iteration',required=True,help='Number of first iteration to analyze')
parser.add_argument('-it_end','--end_iteration',required=True,help='Number of last iteration to analyze')
...
it_start = int(io_args.start_iteration)
it_end = int(io_args.end_iteration)
...
plt.xlim([it_start, it_end])
plt.ylim([0, max(count)+10])
plt.xticks(np.arange(it_start, it_end+0.01, step=1))
